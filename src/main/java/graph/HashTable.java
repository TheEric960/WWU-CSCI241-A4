...
public class HashTable<K,V> {
    ...
    public V put(K key, V val) {
        ...
        buckets[ind] = new Pair(key, val, buckets[ind]);
        size += 1;
        growIfNeeded();
        return null;
    }
    ...
    private void growIfNeeded() {
        double load = (double)size / getCapacity();
        if (load <= 0.8) {
            return;
        }
        Pair[] oldBuckets = buckets; // save old values
        buckets = createBucketArray(oldBuckets.length * 2);
        size = 0;
        for (int i = 0; i < oldBuckets.length; i++) {
            Pair node = oldBuckets[i];
            while (node != null) {
